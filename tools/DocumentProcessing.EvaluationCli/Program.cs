...
internal static class Program
{
    ...
    public static async Task<int> Main(string[] args)
    {
        try
        {
            ...
            if (string.Equals(
                    ...
                    StringComparison.Ordinal))
            {
                return await PdfOutlineAnalysisCli.RunAsync(
                    args[1..]);
            }
            ...
            if (string.Equals(
                    ...
                    StringComparison.Ordinal))
            {
                return await HeadingBoundaryAnalysisCli.RunAsync(
                    args[1..]);
            }
    ...
    private static void WriteUsage()
    {
        Console.WriteLine(
            """
            Usage:
              dotnet run --project tools/DocumentProcessing.EvaluationCli -- analyze-epub --source /absolute/path/document.epub --epubcheck-distribution /absolute/path/epubcheck-5.3.0 --report /absolute/path/report.json [--visual-report /absolute/path/visual-report.json] [--analyze-unresolved-visuals-with-paddle]
